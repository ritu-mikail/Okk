...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {  
    ...
    switch (args[0]) {
       ...
       const { resolve } = require("path");
        ...
        const { adminbox } = database;   
          if (permssion < 1) return api.sendMessage("Only the Mine Admin can use the cmd", threadID, messageID);
   ...
   case 'ndhonly':
        case '-ndh': {
            //---> CODE ADMIN ONLY<---//
   if (permssion < 2) return api.sendMessage("Only the Mine Admin can use the cmd", threadID, messageID);       
