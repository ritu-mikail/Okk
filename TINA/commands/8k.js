...
module.exports.run = async function({ api, event, args }) {
  ...
  try {
    if (isLink) {
      ...
      const res = await axios.get(`https://smfahim.onrender.com/4k?link=${encodeURIComponent(linkUp)}`);
      const link = res.data.uploaded.image;
      const response = await axios.get(`https://smfahim.onrender.com/4k?link=${link}`, { responseType: "arraybuffer" });
