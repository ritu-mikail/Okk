...
(async function () {
  ...
  logger(`${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "THIS FILE FIXED BY CHORU TIKTOKERS" : global.config.BOTNAME}`, "[ BOT INFO ]");
  ...
  return (event) => {
    ...
    let data = JSON.parse(fs.readFileSync(__dirname + "/../TINA/commands/cache/approvedThreads.json"));
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        ...
        return api.sendMessage(`✔ APNAR 𝐑𝐄𝐐𝐔𝐄𝐒𝐓 𝐀𝐃𝐌𝐈𝐍 er kache pathano HoLo ♥️\n\n ===== 𝐍𝐀𝐙𝐑𝐔𝐋 𝐁𝐎𝐓 =====`, event.threadID);
      }
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`⚠️ আপনার গ্রুপের বা ইনবক্স এর এপ্রভাল নাই\n 𝐍𝐀𝐙𝐑𝐔𝐋 𝐁𝐎𝐓 ব্যাবহার করতে হলে! আপনার আগে পারমিশন লাগবে।\n পারমিশন নিতে 𝐍𝐀𝐙𝐑𝐔𝐋 কে ইনবক্স করুন\n গ্রুপে এড করো অথবা\n 💫/request💫\nকমান্ড টা ব্যাবহার করেন ধন্যবাদ🥀🥀।\n\n${prefix}request\n\n✅🥀💯\n\n𝐎𝐰𝐧𝐞𝐫 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐋𝐢𝐧𝐤\nhttps://m.me/ji.la.pi.6\n\n===== 𝐍𝐀𝐙𝐑𝐔𝐋 𝐁𝐎𝐓 =====`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":
      ...
      case "message_reaction":
        if(event.senderID == api.getCurrentUserID() && event.senderID == "100000350435589" && event.reaction == '🥀') {
